...
export default function QuestionCard({
  ...
  return (
    <details className="group rounded-2xl border border-navy/10 bg-white p-5" open={isEmployer}>
      <summary className="flex cursor-pointer list-none items-start justify-between gap-4">
        ...
        <span className="flex flex-shrink-0 items-center gap-2 print:hidden">
          ...
          <span className="rounded-full bg-mist px-2 py-0.5 text-[11px] font-medium text-navy/60">Lvl {q.difficulty}</span>
        </span>
      </summary>
      <div className="mt-4 space-y-3 border-t border-navy/10 pt-4 text-sm text-navy/75">
        ...
        {isEmployer && (
          <div className="grid grid-cols-5 gap-2 border-t border-navy/10 pt-3 print:mt-2">
            <span className="col-span-5 text-xs font-semibold uppercase tracking-wide text-navy/50">Score</span>
            {[1, 2, 3, 4, 5].map((n) => (
              <span
                key={n}
