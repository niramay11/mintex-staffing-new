  ...
  return (
    <div>
      <div className="flex justify-center">
        <div className="inline-flex gap-1 rounded-full bg-mist p-1.5 dark:bg-navy-900">
          <button
            type="button"
            onClick={() => setTab("employers")}
            className={`rounded-full px-5 py-2.5 text-sm font-semibold transition-colors ${
              tab === "employers"
                ? "bg-navy text-white shadow-sm dark:bg-steel dark:text-navy-950 dark:hover:bg-steel-light"
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {services.map((service) => (
          <Link
            key={service.title}
            href={service.href}
            className="rounded-2xl border border-navy/[0.08] p-7 transition-all hover:-translate-y-1 hover:shadow-[0_20px_40px_-20px_rgba(0,48,96,0.25)] dark:border-white/10"
          >
