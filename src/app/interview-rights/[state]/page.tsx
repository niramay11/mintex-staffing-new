    ...
    <>
      ...
      <Section background="cream">
        {rights.isFederalOnly && (
          <p className="mb-6 rounded-xl bg-steel/10 px-4 py-3 text-sm text-navy/70 dark:text-cream/70">{FEDERAL_DISPLAY_NOTICE}</p>
        )}

        <div className="space-y-4">
          {rights.cannot_be_asked.map((item) => (
            <div key={item.question} className="rounded-2xl border border-navy/10 bg-white p-5 dark:border-white/10 dark:bg-navy-800">
        ...
        {rights.state_specific.length > 0 && (
          <div className="mt-8 rounded-2xl border border-steel/40 bg-steel/[0.08] p-6">
            <h2 className="font-heading text-lg font-semibold text-navy dark:text-cream">More about {state}&apos;s protections</h2>
            <ul className="mt-3 space-y-2.5 text-sm text-navy/70 dark:text-cream/70">
        ...
        {rights.legally_confused.length > 0 && (
          <div className="mt-8">
            <h2 className="font-heading text-lg font-semibold text-navy dark:text-cream">Often assumed illegal — but isn&apos;t</h2>
            <div className="mt-3 space-y-3">
              {rights.legally_confused.map((item) => (
                <div key={item.question} className="rounded-2xl border border-navy/10 bg-white p-5 dark:border-white/10 dark:bg-navy-800">
                  <p className="font-medium text-navy dark:text-cream">{item.question}</p>
                  <p className="mt-1 text-sm text-navy/70 dark:text-cream/70">{item.why}</p>
                  <p className="mt-1 text-sm text-navy/70 dark:text-cream/70">{item.guidance}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <p className="mt-8 text-sm text-navy/50 dark:text-cream/50">
          ...
          <Link href="/resources/ai-interview-generator" className="font-medium text-steel hover:text-navy dark:text-steel-light dark:hover:text-cream">
