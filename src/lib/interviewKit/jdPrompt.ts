...
{
  "jobTitle": string,
  "seniority": string | null,    // e.g. "entry", "mid", "senior", "lead" — however the posting actually phrases it, or null if unstated
  "state": string | null,        // US state or location as stated in the posting (e.g. "New Jersey", "Remote (US)"), or null if unstated
...
export function buildJdKitPrompt(extraction: JobDescriptionExtraction, input: GenerateKitInput): string {
  const { seniority, state, focus, industryName } = input;
...
REQUESTED (authoritative — use these for calibration and scoring, even if the
posting's own wording below differs):
  Seniority: ${seniority}
  State: ${state}
  ${focus ? `Focus emphasis: ${focus}` : ""}
${
  extraction.seniority || extraction.state
    ? `\nThe posting itself says: ${extraction.seniority ? `seniority "${extraction.seniority}"` : ""}${extraction.seniority && extraction.state ? ", " : ""}${extraction.state ? `location "${extraction.state}"` : ""}. Use this for flavor/grounding only — the REQUESTED values above are what the candidate actually asked for and take precedence if they differ.`
    : ""
}
...
7. Leave your_rights.cannot_be_asked, your_rights.state_specific, and
   your_rights.legally_confused as EMPTY arrays — no exceptions, that
   content is injected afterward from a verified dataset, never generated
   by you.
